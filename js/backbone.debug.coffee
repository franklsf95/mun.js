...
class ConsoleLogger
  log: (type, object, details) ->
    ...
    if type == 'instance'
      ...
      symbol = "[#{type}]"
    console.log symbol, details, object
...
class Backbone.Debug
  ...
  _onViewBinding: (type, method, object) =>
    ...
    name = @_prettyInstanceName(object, type)
    @logger.log('view', name, object.$el.selector) if @_options['log:views']

  _onEvent: (type, method, object, args) =>
    @logger.log('event', object.constructor.name, args[0]) if @_options['log:events']
